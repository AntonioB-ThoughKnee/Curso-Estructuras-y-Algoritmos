...
//template <typename TipoNodo>
void ArregloPadre::PonerRaiz(std::string etiqueta) {
...
//template <typename TipoNodo>
int ArregloPadre::AgregarHijo(int nodoPadre, std::string etiqueta) {
...
int ArregloPadre::AgregarHijoMasDerecho(int nodoPadre, std::string etiqueta) {
  auto it = this->arreglo.end();
  int nuevo_nodo_id = this->arreglo.size();
...
std::string ArregloPadre::Etiqueta(int nodo_id_externo) {
  return this->arreglo[nodo_id_externo-1].etiqueta;
}

void ArregloPadre::ModificaEtiqueta(std::string etiqueta, int nodo_id_externo) {
